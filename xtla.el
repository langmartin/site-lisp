      ...
      (with-current-buffer buffer
        (if verbose-format
          ...
          (while (or (eq (char-after) ?*)
                     ;;
                     ;; Handle prefix marks for lines here.
                     ;;
                     (looking-at "^\\(.\\)\\([ /Abfl>-]?\\) +\\([^\t\n]*\\)\\(\t\\(.*\\)\\)?$"))
            (if (eq (char-after) ?*)
  ...
  (let ((a-list tla--archive-tree)
    ...
    (while a-list
      (setq archive-name (caar a-list)
            archive-location (car (cdar a-list))
            a-list (cdr a-list)
...
of a .rej file)."
  (when (tla-file-has-conflict-p (buffer-file-name))
    (tla--funcall-if-exists smerge-mode 1)
    (message
     "Conflicts in file, entering SMerge mode. Use M-x {tla|baz}-conflicts-finish RET when done."))
