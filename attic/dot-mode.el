  ...
  (let ((map (make-sparse-keymap)))
    (if (fboundp 'read-kbd-macro)
        (progn
          ...
          ;(define-key map (read-kbd-macro "C-x .") 'dot-mode-override)
...
(cond ((and (fboundp 'event-modifiers) (fboundp 'event-basic-type))
       (defun dot-mode-event-to-string (ev)
         ...
         (let ((em (event-modifiers ev))
               (control-only t))
           (mapcar (lambda (x) (if (not (equal (symbol-name x) "control"))
                                   (setq control-only nil))) em)
           (if control-only
               (char-to-string ev)
             (concat (mapconcat (lambda (x)
                                  (concat "<" (symbol-name x) ">"))
                                em "")
                     (let ((eb (event-basic-type ev)))
